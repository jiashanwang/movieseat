		...
		<view class="bg-f1 h-100vh">
			...
			<view style="padding-top: 40rpx; box-sizing:  border-box;">
				<view class="dp-f jc-c ai-c mb-20 fz-28" v-if="showTis">
					...
					<view class="dp-f jc-c ai-c m-0-10">
						<image :style="'width:'+seatSize+'px;height:'+seatSize+'px'" src="/static/images/bought.png"
							mode="aspectFit"></image><span class="ml-10">不可选</span>
					</view>
					<view class="dp-f jc-c ai-c m-0-10">
						<image :style="'width:'+seatSize+'px;height:'+seatSize+'px'" src="/static/images/selected.png"
			...
			<movable-area :style="'height:'+(seatRow*40+350)+'rpx;width: 100vw;top:'+(rpxNum*132)+'px;margin-top: 5vw;'"
				class="pt-f left-0">
				<movable-view :style="'width: 100vw;height:'+(seatRow*40+350)+'rpx;'" :inertia="true" :scale="true"
					...
					<view v-for="(item,index) in seatArray" :key="index" class="dp-f jc-c mt-20"
						...
						<view v-for="(seat,col) in item" :key="col" class="dp-ib"
							...
							<!-- <image v-if="seat.type===0" class="w-100 h-100" :src="../../static/images/unselected.png"
								mode="aspectFit"></image> -->

							<image v-if="seat.type===0" class="w-100 h-100" :src="seat.icon" mode="aspectFit"></image>
							<!-- seat.SeatCode == ids && seat.SeatCode ==ids -->
							<image v-else-if="seat.type===1  " class="w-100 h-100"
								src="../../static/images/selected.png" mode="aspectFit">

							</image>
							<image v-else-if="seat.type===2" class="w-100 h-100" src="../../static/images/bought.png"
								mode="aspectFit"></image>
						</view>
					</view>
					<!-- mArr -->
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.show_time = options.show_time
			this.screens = uni.getStorageSync('screens')
			// console.log(options,'-------------------')
			if (options.seatCount != undefined) {
				this.seatCount = parseInt(options.seatCount);
				this.seatCountInit = parseInt(options.seatCount);
			}
			//获取座位
			this.getseatSelec();
			//配置
		...
		methods: {
			...
			async refreshUserInfo() {
				...
				//刷新
				if (res.data.centerMember != null) {
					this.userInfo = res.data.centerMember;
					//用户会员卡
					if (res.data.centerMemberCard) {
						this.userInfoCard = res.data.centerMemberCard;
					}
				}
			},
			/**
			...
			async companySetting() {
				...
				//设置
				if (res.data.centerWapAccount != null) {
					this.wxSamllConfig = res.data.centerWapAccount
				}
			},
			/**
			 * 分转元
			 ...
			 */
			matching(obj, key) {
				let icon = "/static/images/unselected.png";
				...
				uni.request({
					...
					success: (resp) => {
						...
						if (res.code == 200) {
							...
							let arrList = res.data.cinemaSchedulesList;
							...
							let priceGrouping = res.data.priceGrouping;
							
							this.priceGrouping = [];
							let count = 0;
							//循环遍历
							for (let key in priceGrouping) {
								// console.log("---------------------", key);
								let obj = {
									key: key,
									value: this.spunYuan(key) + "元",
									icon: "/static/images/selected" + count + ".png"
								}
								this.priceGrouping.push(obj);
								count++;
							}
							
							// console.log(res.data.cinemaSchedulesList[0])
							let list = [];
							...
							for (let i = 0; i < arrList.length; i++) {
								let pobj = arrList[i];
								let obj = {
									"icon": this.matching(pobj, pobj.price),
									"YCoord": pobj.row,
									"XCoord": pobj.column,
									"SeatCode": pobj.ext_id,
									"Status": pobj.status == 0 ? 0 : 1,
									"exId": pobj.ext_id,
									"flag": pobj.flag,
									"RowNum": pobj.row,
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							//授权
							let isFalg = uni.getStorageSync('tmplIds');
							//如果已授权，则缓存没消失之前不进行授权处理
							if (isFalg == undefined || isFalg == null || isFalg == "") {
								//默认进行授权
								if (that.tmplIds != "") {
									uni.requestSubscribeMessage({
										...
										success: (res) => {
											//授权
											uni.setStorageSync('tmplIds', "yes");
											//进行提交
											that.saveFilmTicket()
										},
									})
			...
			saveFilmTicket() {
				...
				//原价
				data.displayPrice = this.spunYuan(price);
				//非会员价格
